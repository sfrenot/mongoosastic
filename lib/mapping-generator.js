...
function nestedSchema(paths, field, cleanTree, value, prefix) {
  // A nested array can contain complex objects
  function getInstance(fieldVar){
    if(paths[fieldVar]){
      if(paths[fieldVar].caster){
        return paths[fieldVar].caster.instance || paths[fieldVar].instance;
      } else  {
        return paths[fieldVar].instance;
      }
    }
  }
  if (paths[field] && paths[field].schema && paths[field].schema.tree && paths[field].schema.paths) {
  ...
  } else if (paths[field] && paths[field].caster && getInstance(field)){
    ...
    cleanTree[field].type = getInstance(field).toLowerCase();
  } else if (!paths[field] && prefix) {
    if (paths[prefix + field] && paths[prefix + field].caster && getInstance(prefix + field)) {
      cleanTree[field] = {
        type: getInstance(prefix + field).toLowerCase()
      };
    }
  } else {
    cleanTree[field] = {
